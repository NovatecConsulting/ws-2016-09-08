...
public class Iso7064CodeGenerator {
    ...
    public long calculateISO7064CheckCode(CountryCode country, String bankCode, String accountNumber) {
        String ibanForCheckCode = String.format("%s%s%s00", bankCode, accountNumber, ALPHA_CONVERSION_TABLE.get(country));
        BigInteger checkCodeBase = new BigInteger(ibanForCheckCode);
        long checkCode = 98L - checkCodeBase.mod(BigInteger.valueOf(97L)).longValue();
        return checkCode;
    }
}
